...
public class EditCommandParserTest {
    ...
    public void parse_missingParts_failure() {
        ...
        // no field specified
        assertParseFailure(parser, "1", EditCommand.MESSAGE_NO_FIELDS);
    ...
    public void parse_invalidValue_failure() {
        assertParseFailure(parser, "1" + INVALID_NAME_SYMBOL_DESC, Name.MESSAGE_CONSTRAINTS); // invalid name
        ...
        assertParseFailure(parser, "1" + TAG_DESC_AMY + TAG_DESC_AMY_2 + TAG_EMPTY, Tag.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_DESC_AMY + TAG_EMPTY + TAG_DESC_AMY_2, Tag.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_EMPTY + TAG_DESC_AMY + TAG_DESC_AMY_2, Tag.MESSAGE_CONSTRAINTS);

        // multiple invalid values, but only the first invalid value is captured
        assertParseFailure(parser, "1" + INVALID_NAME_SYMBOL_DESC + INVALID_EMAIL_DESC + VALID_ADDRESS_AMY
                        + VALID_PHONE_AMY,
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + TAG_DESC_AMY
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + TAG_DESC_AMY;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_AMY).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withTags(VALID_TAG_AMY, VALID_TAG_AMY).build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // shifts
        userInput = targetIndex.getOneBased() + SHIFTS_DESC_CARL;
        descriptor = new EditPersonDescriptorBuilder().withNote(VALID_SHIFTS_CARL).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // items
        userInput = targetIndex.getOneBased() + ITEMS_DESC_ELLE;
        descriptor = new EditPersonDescriptorBuilder().withNote(VALID_ITEMS_ELLE).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // days
        userInput = targetIndex.getOneBased() + DAYS_DESC_ELLE;
        descriptor = new EditPersonDescriptorBuilder().withNote(VALID_DAYS_ELLE).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_failure() {
        ...
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + INVALID_PHONE_DESC;

        assertParseFailure(parser, userInput, Messages.getErrorMessageForDuplicatePrefixes(PREFIX_PHONE));

        // mulltiple valid fields repeated
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + TAG_DESC_AMY + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY + TAG_DESC_AMY
                + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY + TAG_DESC_AMY_2;
